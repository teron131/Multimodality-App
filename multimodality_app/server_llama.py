...
from .llm_llama import get_response
...
# FastAPI app
app = FastAPI(title="Multimodality App - Llama CUDA", description="Multimodal AI application using local llama-cuda server")
...
class ConfigResponse(BaseModel):
    ...
    server_type: str = "llama-cuda"
    model: str = "ultravox-v0_5-llama-3_2-1b"
    server: str = "multimodality-app-llama"


class StatusResponse(BaseModel):
    server_status: str = "running"
    message: str = "Audio processing ready with llama-cuda"
    llama_server: str = "http://localhost:8081"
...
async def get_config():
    """Get API configuration for llama-cuda server."""
...
@app.post("/api/process-audio", response_model=LlamaResponse)
async def process_audio(request: LlamaRequest):
    """Process audio with llama-cuda server."""
    try:
        # Use llama-cuda LLM module
        response = get_response(text_input=request.prompt, audio_base64=request.audio_base64)

        # Convert LangChain response to expected format for compatibility
        llama_response = {
        ...
        return LlamaResponse(status="success", llama_response=llama_response)

    except ConnectionError as e:
        logger.error(f"Cannot connect to llama-cuda server: {e}")
        raise HTTPException(status_code=503, detail=str(e))
    except Exception as e:
        logger.error(f"Error processing audio with llama-cuda: {e}")
...
async def process_audio_unified(audio: UploadFile = File(...), prompt: str = "Please transcribe this audio recording and provide any additional insights about what you hear."):
    ...
    try:
        ...
        # Use llama-cuda LLM module for processing
        response = get_response(text_input=prompt, audio_base64=audio_base64)

        return UnifiedProcessResponse(status="success", message="Audio processed successfully with llama-cuda", transcription=response.content, size_bytes=len(audio_data))
...
async def process_image_unified(image: UploadFile = File(...), prompt: str = "Please analyze this image and describe what you see."):
    ...
    try:
        ...
        # Use llama-cuda LLM module for processing with image_base64
        response = get_response(text_input=prompt, image_base64=image_base64)

        return MultimodalResponse(status="success", message="Image processed successfully with llama-cuda", analysis=response.content, content_type="image", size_bytes=len(image_data))
...
async def process_multimodal_unified(audio: UploadFile = File(None), image: UploadFile = File(None), prompt: str = "Please analyze this content and provide insights."):
    ...
    try:
        ...
        # Use llama-cuda LLM module for multimodal processing
        response = get_response(text_input=prompt, audio_base64=audio_base64, image_base64=image_base64)

        return MultimodalResponse(status="success", message=f"Multimodal processing successful with llama-cuda ({' + '.join(content_types)})", analysis=response.content, content_type=" + ".join(content_types), size_bytes=total_size)
...
async def health_check():
    ...
    try:
        ...
        return {"status": "degraded", "llama_server": "disconnected", "message": "llama-cuda server not available"}
    except Exception as e:
...
if __name__ == "__main__":
    ...
    uvicorn.run("multimodality_app.server_llama:app", host="127.0.0.1", port=3031, reload=True)
